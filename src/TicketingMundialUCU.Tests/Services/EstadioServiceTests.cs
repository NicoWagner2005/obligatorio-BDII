...
public sealed class EstadioServiceTests
{
    ...
    private readonly IEstadioDao _dao = Substitute.For<IEstadioDao>();
    ...
    public EstadioServiceTests()
    {
        ...
        var jurisdictionService = new AdministratorJurisdictionService(
            jurisdictionDao,
            currentUser);
        _service = new EstadioService(_dao, jurisdictionService);
    }
    ...
    public async Task RegistrarEstadio_usa_el_pais_del_administrador()
    {
        ...
        await _dao.Received(1).CreateEstadioAsync("Azteca", Country, sectores);
    }
    ...
    {
        var sectores = CrearSectoresValidos();
        _dao.UpdateEstadioAsync(10, "Azteca", Country, sectores).Returns(false);
    ...
    public async Task ActualizarEstadio_con_capacidad_no_positiva_rechaza_la_operacion()
    {
        ...
        Assert.Equal("La capacidad de cada sector debe ser mayor a 0.", exception.Message);
        await _dao.DidNotReceiveWithAnyArgs().UpdateEstadioAsync(
            default,
            default!,
            default!,
            default!);
    ...
    {
        _dao.DeleteEstadioAsync(8, Country).Returns(false);
